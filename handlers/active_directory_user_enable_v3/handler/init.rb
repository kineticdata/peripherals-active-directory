...
class ActiveDirectoryUserEnableV2
  ...
  def execute()
    ...
    if @ldap.bind
      ...
      if @debug_logging_enabled
        mask = 1 << 1
        if mask & uac == 0
          puts ("User account was not enabled") if @debug_logging_enabled
        else
          puts ("User account was enabled") if @debug_logging_enabled
        end
      end
      # regardless of the status, enable using bitwise operator
      # 0x2 disables the account so we want the NOT operator
      val = uac & ~0x2
