...
class ActiveDirectoryUserExpirePasswordV3
  ...
  def execute()
    begin
      ...
      if @ldap.bind
        ...
        puts("found user record #{user_entries[0].dn}") if @debug_logging_enabled
        # set the password last set to 0 which will expire the account
        @ldap.replace_attribute(user_entries[0].dn, :pwdlastset, '0')
        # Raise an exception if there was a problem with the call
        unless @ldap.get_operation_result.code == 0
