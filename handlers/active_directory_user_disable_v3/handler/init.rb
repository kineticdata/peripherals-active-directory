...
class ActiveDirectoryUserDisableV3
  ...
  # parameters starting with a filter for organizationalPerson and Disables the
  # user's account.
  #
  # This is a required method that is automatically called by the Kinetic Task
  # Engine.
  ...
  def execute()
    begin
      ...
      if @ldap.bind
        ...
        # determine the status of the account for loggin
        if @debug_logging_enabled
          mask = 1 << 1
          if mask & uac == 0
            puts ("User account was disabled") if @debug_logging_enabled
          else
            puts ("User account was already disabled") if @debug_logging_enabled
          end
        end
        ...
        # regardless of the status, disable using bitwise operator
        # 0x2 disables the account so we want the bitwise or operator
        val = uac | 0x2
        # set the attribute back which will disable the account
