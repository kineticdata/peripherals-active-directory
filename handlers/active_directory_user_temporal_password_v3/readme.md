...
Address and unchecks the 'password never expires' flag.  This handler will fail
if the user is not found, or if more than one result is found.
...
Finally, the flag is unset for 'password never expires' so that the password
will expire.
