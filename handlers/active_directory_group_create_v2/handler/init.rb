...
class ActiveDirectoryGroupCreateV2
  ...
  def get_group_type(scope, type)
    if scope == "Global" : scope_value = ADS_GROUP_TYPE_GLOBAL_GROUP
    elsif scope == "Domain local" : scope_value = ADS_GROUP_TYPE_LOCAL_GROUP
    elsif scope == "Universal" : scope_value = ADS_GROUP_TYPE_UNIVERSAL_GROUP
    end
    if type == "Security" : type_value = ADS_GROUP_TYPE_SECURITY_ENABLED
    else type_value = 0
    end
