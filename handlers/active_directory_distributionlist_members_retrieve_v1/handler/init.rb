...
class ActiveDirectoryDistributionlistMembersRetrieveV1
  ...
  def getDistributionListDN()
    ...
    if dl_entries.length < 1
      raise "Distribution List not found when searching by mail for: #{@parameters['search_value']}"
    elsif dl_entries.length > 1
      raise "More than one Distribution List was found when searching by mail for: #{@parameters['search_value']}"
    end
