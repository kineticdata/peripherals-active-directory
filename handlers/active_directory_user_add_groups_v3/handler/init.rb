...
class ActiveDirectoryUserAddGroupsV3
  ...
  def initialize(input)
    ...
    if @parameters['search_by'] == "User Logon"
      ...
      @error_message = "Unknown search attribute for user: #{@parameters['search_by']}"
      raise "Unknown search attribute for user: #{@parameters['search_by']}" if @error_handling == "Raise Error"
    end
  ...
  def execute()
    begin
      ...
      # If authentication of the ldap session failed
        ...
        @error_message = "Directory authentication failed for #{@info_values['host']}: #{@ldap.get_operation_result}"
        # Raise an error
